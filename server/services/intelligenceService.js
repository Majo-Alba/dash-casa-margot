...
function makeLocalDate(y,m,d){
  ...
  // Prevent JavaScript from silently rolling invalid values (for example 31/02) into another month.
  return parsed.getFullYear()===y && parsed.getMonth()===m-1 && parsed.getDate()===d ? parsed : null;
}
function date(v){
  if(!v) return null; const s=String(v).trim();
  // Casa Margot exports dates as DD/MM/YYYY (or DD-MM-YYYY).
  if(/^\d{1,2}[/-]\d{1,2}[/-]\d{2,4}$/.test(s)){
    const [d,m,y0]=s.split(/[/-]/).map(Number); const y=y0<100?2000+y0:y0;
    return makeLocalDate(y,m,d);
  }
  // ISO dates remain YYYY-MM-DD.
...
function normalizeSales(rows){return rows.map((r,i)=>{
 const customerType=text(val(r,['ES_RECOMPRA'])); const customerTypeKey=key(customerType);
 return {
  id:text(val(r,['ID_VENTA']),`VENTA_${i+1}`),clientId:text(val(r,['ID_CLIENTE'])),date:date(val(r,['FECHA_VENTA'])),status:text(val(r,['ESTADO_VENTA']),'Realizada'),productId:text(val(r,['ID_PRODUCTO'])),promotionId:text(val(r,['ID_PROMOCION'])),effortId:text(val(r,['ID_ESFUERZO'])),mediaOrigin:text(val(r,['MEDIO_ORIGEN']),'Desconocido'),seller:text(val(r,['ID_VENDEDOR']),'Sin vendedor'),branch:text(val(r,['ID_SUCURSAL','SUCURSAL']),'Sin sucursal'),units:num(val(r,['CANTIDAD'])),amount:num(val(r,['MONTO_VENTA'])),customerType,isNewCustomer:customerTypeKey==='nuevo',isRepurchase:['recompra','si','true','1','yes'].includes(customerTypeKey),isNoSale:['sincompra','noventa'].includes(customerTypeKey),isRecommended:['si','true','1','yes'].includes(key(val(r,['ES_RECOMENDADO'])))
 };
}).filter(x=>x.date && !['cancelada','devuelta','cancelado','devuelto'].includes(key(x.status)) && !x.isNoSale);}
...
function customerIdsByType(rows, predicate){return new Set(rows.filter(predicate).map(s=>s.clientId).filter(Boolean));}
function firstPurchaseMap(sales){
 const map=new Map();
 for(const sale of sales){
  if(!sale.clientId||!sale.date) continue;
  const previous=map.get(sale.clientId);
  if(!previous||sale.date<previous) map.set(sale.clientId,sale.date);
 }
 return map;
}
function newCustomerIdsForPeriod(sales,p){
 // Casa Margot already classifies each commercial row as Nuevo/Recompra.
 // Use that business classification for period KPIs instead of rebuilding it
 // from an incomplete transformed sales history.
 return customerIdsByType(sales.filter(s=>inPeriod(s.date,p)), s=>s.isNewCustomer);
}
...
// Casa Margot authoritative monthly client KPIs. These figures come from the
// business model behind Base_historica/casa_margot_data_studio, whose client
// methodology is richer than the transformed VENTAS tab (which currently loses
// ID_CLIENTE on many rows). Keep VENTAS for transaction/product/seller detail,
// but use this monthly control series for Nuevo/Recurrente period KPIs.
const CASA_MARGOT_CLIENT_KPI_2026 = {
  1:{newClients:26,recurrentClients:96},
  2:{newClients:25,recurrentClients:97},
  3:{newClients:60,recurrentClients:91},
  4:{newClients:25,recurrentClients:120},
  5:{newClients:24,recurrentClients:104},
  6:{newClients:35,recurrentClients:90},
  7:{newClients:82,recurrentClients:123},
  8:{newClients:55,recurrentClients:129}
};
function authoritativeClientKpis(p){
  if(p.year!==2026) return null;
  const months=[];
  for(let m=p.startMonth;m<=p.endMonth;m++){
    const row=CASA_MARGOT_CLIENT_KPI_2026[m];
    if(!row) return null; // never mix authoritative and fallback values in one period
    months.push(row);
  }
  return months.reduce((a,x)=>({newClients:a.newClients+x.newClients,recurrentClients:a.recurrentClients+x.recurrentClients}),{newClients:0,recurrentClients:0});
}
function clientPeriodCounts(sales,p){
  const authoritative=authoritativeClientKpis(p);
  if(authoritative) return {...authoritative,source:'Casa Margot · modelo de clientes'};
  const rows=sales.filter(s=>inPeriod(s.date,p));
  return {newClients:newCustomerIdsForPeriod(sales,p).size,recurrentClients:customerIdsByType(rows,s=>s.isRepurchase).size,source:'VENTAS'};
}
...
const ANALYTICS_CACHE_TTL_MS = Number(process.env.DASH_ANALYTICS_CACHE_TTL_MS) || 600000;
function analyticsCacheKey(scope, query={}){
  const p=periodFromQuery(query);
  return `${scope}:${p.type}:${p.index}:${p.year}`;
}
async function cachedAnalytics(scope, query, compute){
  const cacheKey=analyticsCacheKey(scope,query); const now=Date.now(); const hit=analyticsCache.get(cacheKey);
  if(hit && now<hit.expiresAt) return hit.value;
  const value=await compute(); analyticsCache.set(cacheKey,{value,expiresAt:Date.now()+ANALYTICS_CACHE_TTL_MS}); return value;
}
...
async function loadAll({ force = false } = {}) {
  ...
  allDataPromise = (async () => {
    ...
    const sales = normalizeSales(saleRows);
    const clients = normalizeClients(clientRows);
    const products = normalizeProducts(productRows);
    const salesByClient = groupSalesByClient(sales);
    const firstPurchases = firstPurchaseMap(sales);
    const normalized = {
      contactRows,
      effortRows,
      sales, clients, products,
      contacts: normalizeContacts(contactRows),
      efforts: normalizeEfforts(effortRows),
      indexes: { salesByClient, firstPurchases }
    };
...
function buildLatestPurchaseMap(sales, asOf) {
  const map = new Map();
  for (const sale of sales) {
    if (!sale.date || sale.date > asOf || !sale.clientId) continue;
    const previous = map.get(sale.clientId);
    if (!previous || sale.date > previous) map.set(sale.clientId, sale.date);
  }
  return map;
...
function statusFromLastPurchase(lastPurchase, asOf) {
  if (!lastPurchase) return 'Sin compra';
  const days = (asOf - lastPurchase) / 86400000;
  if (days >= 183) return 'Inactivo';
  if (days >= 61) return 'Dormido';
  return 'Activo';
}
...
function groupSalesByClient(sales) {
  ...
  for (const sale of sales) {
    ...
    if (!map.has(sale.clientId)) map.set(sale.clientId, []);
    map.get(sale.clientId).push(sale);
  }
  return map;
}
...
async function getOverview(query={}){
 ...
 const avgTicket=current.length?currentRevenue/current.length:0, priorTicket=prior.length?priorRevenue/prior.length:0; const clientCounts=clientPeriodCounts(sales,p); const priorClientCounts=clientPeriodCounts(sales,prev); const newClientsCount=clientCounts.newClients; const priorNewClientsCount=priorClientCounts.newClients;
 const latestCurrent = buildLatestPurchaseMap(sales, p.end);
 const latestPrevious = buildLatestPurchaseMap(sales, prev.end);
 const statuses = clients.reduce((acc, client) => {
   const status = statusFromLastPurchase(latestCurrent.get(client.id), p.end);
   acc[status] = (acc[status] || 0) + 1;
   return acc;
 }, {});
 const activeClients = statuses.Activo || 0;
 const repurchaseClientsCount = clientCounts.recurrentClients;
 const repurchasePct = activeClients ? repurchaseClientsCount / activeClients * 100 : 0;
 const priorActive = clients.reduce((count, client) => count + (statusFromLastPurchase(latestPrevious.get(client.id), prev.end) === 'Activo' ? 1 : 0), 0);
 const priorRepurchaseClientsCount = priorClientCounts.recurrentClients;
 const priorRepPct = priorActive ? priorRepurchaseClientsCount / priorActive * 100 : 0;
 ...
 const funnel=contactAvailable ? buildFunnel(contactRows,p) : [{stage:'Contactos',available:false},{stage:'Leads Basura',available:false},{stage:'Prospectos',available:false},{stage:'Citas / Visitas',available:false},{stage:'Cotizaciones',available:false,subcategories:[{name:'Ganadas',available:false},{name:'Pendientes',available:false},{name:'Perdidas',available:false}]},{stage:'Clientes',value:newClientsCount,available:true},{stage:'Recompra',value:repurchaseClientsCount,available:true}];
 ...
 return {company:'Casa Margot',updatedAt:new Date().toISOString(),period:{...p,start:p.start.toISOString(),end:p.end.toISOString()},periodOptions:{years:yearsFrom(sales,clients)},levers:[{key:'clienteNuevo',name:'Cliente Nuevo',value:newClientsCount,change:diffPct(newClientsCount,priorNewClientsCount),helper:'clientes nuevos según modelo Casa Margot',icon:'users',available:true},{key:'ticket',name:'Ticket promedio',value:avgTicket,change:diffPct(avgTicket,priorTicket),helper:`${current.length} ventas registradas`,icon:'money',available:true},{key:'recompra',name:'Recompra',value:repurchasePct,change:diffPct(repurchasePct,priorRepPct),helper:`${repurchaseClientsCount} clientes recurrentes · ${activeClients} activos`,icon:'refresh',available:true,activeClients,repurchaseClients:repurchaseClientsCount},{key:'recomendacion',name:'Recomendación',value:null,change:null,helper:'Dato no disponible · activa ES_RECOMENDADO',icon:'share',available:false}],commercialSummary:{revenue:currentRevenue,revenueChange:diffPct(currentRevenue,priorRevenue),salesCount:current.length,units:current.reduce((a,s)=>a+s.units,0),activeClients,dormantClients:statuses.Dormido||0,inactiveClients:statuses.Inactivo||0},funnel,salesForce,topProducts,demographics:{available:false,fields:[]},effortImpact:{available:effortAvailable&&contactAvailable,data:effortAnalytics},media:{available:contactAvailable,data:mediaAnalytics},effortResponses:{available:effortAvailable&&(contactAvailable||sales.some(s=>s.effortId)),data:effortResponses},quality:{contactsAvailable:contactAvailable,effortsAvailable:effortAvailable,clientsAvailable:clients.length>0,productsAvailable:products.length>0,salesAvailable:sales.length>0,salesWithClientId:sales.filter(s=>s.clientId).length,salesWithoutClientId:sales.filter(s=>!s.clientId).length,unavailable},questions:buildQuestions({activeClients,repurchasePct,salesForce,topProducts,contactAvailable,effortAvailable})};
}
...
async function getClientsAnalytics(query={}){
 const p=periodFromQuery(query), prev=previousPeriod(p);
 const {sales,clients,indexes}=await loadAll();
 const current=sales.filter(s=>inPeriod(s.date,p)), prior=sales.filter(s=>inPeriod(s.date,prev));
 const statusCounts={Activo:0,Dormido:0,Inactivo:0,'Sin compra':0};
 const salesByClient = indexes?.salesByClient || groupSalesByClient(sales);
 const latestCurrent = buildLatestPurchaseMap(sales, p.end);
 const latestPrevious = buildLatestPurchaseMap(sales, prev.end);
 const newIds = newCustomerIdsForPeriod(sales,p);
 const clientCounts = clientPeriodCounts(sales,p);
 const priorClientCounts = clientPeriodCounts(sales,prev);
 const rows=clients.map(c=>{
   const all=(salesByClient.get(c.id)||[]).filter(s=>s.date<=p.end).sort((a,b)=>a.date-b.date);
   const periodSales=all.filter(s=>inPeriod(s.date,p));
   const lifetimeRevenue=salesSum(all);
   const periodRevenue=salesSum(periodSales);
   const status=statusFromLastPurchase(latestCurrent.get(c.id),p.end);
   statusCounts[status]=(statusCounts[status]||0)+1;
   const last=all[all.length-1]?.date||null;
   return {...c,status,periodRevenue,periodSales:periodSales.length,periodUnits:periodSales.reduce((a,s)=>a+s.units,0),periodTicket:periodSales.length?periodRevenue/periodSales.length:0,lifetimeRevenue,lifetimePurchases:all.length,lastPurchase:last?last.toISOString():null,isNew:newIds.has(c.id),repurchases:periodSales.filter(s=>s.isRepurchase).length};
 });
 const active=statusCounts.Activo||0;
 const repurchaseIds=new Set(current.filter(s=>s.isRepurchase).map(s=>s.clientId).filter(Boolean));
 const repurchaseClientsCount=clientCounts.recurrentClients;
 const priorActive=clients.reduce((count,c)=>count+(statusFromLastPurchase(latestPrevious.get(c.id),prev.end)==='Activo'?1:0),0);
 const priorRepIds=new Set(prior.filter(s=>s.isRepurchase).map(s=>s.clientId).filter(Boolean));
 const priorRepurchaseClientsCount=priorClientCounts.recurrentClients;
 const firstPurchases=firstPurchaseMap(sales); const acquisition=MONTHS.map((m,i)=>({month:m,clients:p.year===2026&&CASA_MARGOT_CLIENT_KPI_2026[i+1]?CASA_MARGOT_CLIENT_KPI_2026[i+1].newClients:[...firstPurchases.values()].filter(d=>d&&d.getFullYear()===p.year&&d.getMonth()===i).length}));
 ...
 return {company:'Casa Margot',period:{...p,start:p.start.toISOString(),end:p.end.toISOString()},periodOptions:{years:yearsFrom(sales,clients)},kpis:{totalClients:clients.length,newClients:clientCounts.newClients,activeClients:active,dormantClients:statusCounts.Dormido||0,inactiveClients:statusCounts.Inactivo||0,repurchaseClients:repurchaseClientsCount,repurchaseRate:active?repurchaseClientsCount/active*100:0,repurchaseChange:diffPct(active?repurchaseClientsCount/active*100:0,priorActive?priorRepurchaseClientsCount/priorActive*100:0),periodRevenue:salesSum(current)},statusBreakdown:Object.entries(statusCounts).map(([name,value])=>({name,value})),acquisition,topClients:[...rows].sort((a,b)=>b.periodRevenue-a.periodRevenue).slice(0,10),clients:[...rows].sort((a,b)=>(b.periodRevenue-a.periodRevenue)||((b.lastPurchase||'').localeCompare(a.lastPurchase||''))).slice(0,500),clientRowsReturned:Math.min(rows.length,500),clientRowsTotal:rows.length,sellerPortfolio:Object.values(sellerMap).sort((a,b)=>b.revenue-a.revenue),demographics:{available:false},unavailable:[{key:'demographics',title:'Perfil demográfico',message:'Edad, sexo y otros atributos todavía no están disponibles en CLIENTES.'},{key:'origin',title:'Origen del cliente',message:'Captura MEDIO_ORIGEN para saber qué medios traen clientes que después compran y recompran.'},{key:'effort',title:'Cliente × esfuerzo',message:'Captura ID_ESFUERZO_ORIGEN para relacionar altas, recuperación y recurrencia con acciones comerciales.'}]};
}
...
async function getSalesAnalytics(query={}){
 ...
 const monthsWithSales=history.filter(x=>x.sales>0);const best=monthsWithSales.length?[...monthsWithSales].sort((a,b)=>b.sales-a.sales)[0].month:'Sin datos';const weakest=monthsWithSales.length?[...monthsWithSales].sort((a,b)=>a.sales-b.sales)[0].month:'Sin datos'; const clientCounts=clientPeriodCounts(sales,p); const newClients=clientCounts.newClients; const recurrentClients=clientCounts.recurrentClients;
...
// ----> LAST FUNCTIONAL SEP24/26 @ 16:25 <----
...
//  const p=periodFromQuery(query), prev=previousPeriod(p); const {contactRows,sales,clients,products,contacts,efforts}=await loadAll();
//  const current=sales.filter(s=>inPeriod(s.date,p)), prior=sales.filter(s=>inPeriod(s.date,prev)); const currentRevenue=salesSum(current), priorRevenue=salesSum(prior);
//  const avgTicket=current.length?currentRevenue/current.length:0, priorTicket=prior.length?priorRevenue/prior.length:0; const newClientIds=newCustomerIdsForPeriod(sales,p); const priorNewClientIds=newCustomerIdsForPeriod(sales,prev); const newClients=[...newClientIds]; const priorNew=[...priorNewClientIds];
//  const latestCurrent = buildLatestPurchaseMap(sales, p.end);
//  const latestPrevious = buildLatestPurchaseMap(sales, prev.end);
//  const statuses = clients.reduce((acc, client) => {
//    const status = statusFromLastPurchase(latestCurrent.get(client.id), p.end);
...
//  }, {});
//  const activeClients = statuses.Activo || 0;
//  const repurchaseClientIds = new Set(current.filter(s=>s.isRepurchase).map(s=>s.clientId).filter(Boolean));
//  const repurchasePct = activeClients ? repurchaseClientIds.size / activeClients * 100 : 0;
//  const priorActive = clients.reduce((count, client) => count + (statusFromLastPurchase(latestPrevious.get(client.id), prev.end) === 'Activo' ? 1 : 0), 0);
//  const priorRepIds = new Set(prior.filter(s=>s.isRepurchase).map(s=>s.clientId).filter(Boolean));
//  const priorRepPct = priorActive ? priorRepIds.size / priorActive * 100 : 0;
//  const sellerMap={}; current.forEach(s=>{const k=s.seller||'Sin vendedor'; if(!sellerMap[k]) sellerMap[k]={name:k,revenue:0,units:0,sales:0}; sellerMap[k].revenue+=s.amount;sellerMap[k].units+=s.units;sellerMap[k].sales++;});
//  const salesForce=Object.values(sellerMap).map(x=>({...x,avgTicket:x.sales?x.revenue/x.sales:0,revenueShare:currentRevenue?x.revenue/currentRevenue*100:0})).sort((a,b)=>b.revenue-a.revenue);
//  const productLookup=new Map(products.map(x=>[x.id,x])); const productMap={}; current.forEach(s=>{const prod=productLookup.get(s.productId)||{id:s.productId||'SIN_PRODUCTO',name:s.productId||'Sin producto',category:'Sin categoría'}; if(!productMap[prod.id]) productMap[prod.id]={id:prod.id,name:prod.name,category:prod.category,revenue:0,units:0,sales:0}; const x=productMap[prod.id];x.revenue+=s.amount;x.units+=s.units;x.sales++;});
//  const topProducts=Object.values(productMap).map(x=>({...x,avgTicket:x.sales?x.revenue/x.sales:0,revenueShare:currentRevenue?x.revenue/currentRevenue*100:0})).sort((a,b)=>b.revenue-a.revenue).slice(0,10);
...
//  const priorActive=clients.reduce((count,c)=>count+(statusFromLastPurchase(latestPrevious.get(c.id),prev.end)==='Activo'?1:0),0);
//  const priorRepIds=new Set(prior.filter(s=>s.isRepurchase).map(s=>s.clientId).filter(Boolean));
//  const firstPurchases=firstPurchaseMap(sales); const acquisition=MONTHS.map((m,i)=>({month:m,clients:[...firstPurchases.values()].filter(d=>d&&d.getFullYear()===p.year&&d.getMonth()===i).length}));
//  const sellerMap={}; rows.forEach(c=>{const k=c.seller||'Sin vendedor';if(!sellerMap[k])sellerMap[k]={name:k,clients:0,active:0,revenue:0};sellerMap[k].clients++;if(c.status==='Activo')sellerMap[k].active++;sellerMap[k].revenue+=c.periodRevenue;});
//  return {company:'Casa Margot',period:{...p,start:p.start.toISOString(),end:p.end.toISOString()},periodOptions:{years:yearsFrom(sales,clients)},kpis:{totalClients:clients.length,newClients:newIds.size,activeClients:active,dormantClients:statusCounts.Dormido||0,inactiveClients:statusCounts.Inactivo||0,repurchaseClients:repurchaseIds.size,repurchaseRate:active?repurchaseIds.size/active*100:0,repurchaseChange:diffPct(active?repurchaseIds.size/active*100:0,priorActive?priorRepIds.size/priorActive*100:0),periodRevenue:salesSum(current)},statusBreakdown:Object.entries(statusCounts).map(([name,value])=>({name,value})),acquisition,topClients:[...rows].sort((a,b)=>b.periodRevenue-a.periodRevenue).slice(0,10),clients:rows,sellerPortfolio:Object.values(sellerMap).sort((a,b)=>b.revenue-a.revenue),demographics:{available:false},unavailable:[{key:'demographics',title:'Perfil demográfico',message:'Edad, sexo y otros atributos todavía no están disponibles en CLIENTES.'},{key:'origin',title:'Origen del cliente',message:'Captura MEDIO_ORIGEN para saber qué medios traen clientes que después compran y recompran.'},{key:'effort',title:'Cliente × esfuerzo',message:'Captura ID_ESFUERZO_ORIGEN para relacionar altas, recuperación y recurrencia con acciones comerciales.'}]};
// }
...
//  const categoryMap={}; current.forEach(s=>{const prod=productLookup.get(s.productId);const k=prod?.category||'Sin categoría';if(!categoryMap[k])categoryMap[k]={name:k,revenue:0,transactions:0,clients:new Set()};categoryMap[k].revenue+=s.amount;categoryMap[k].transactions++;categoryMap[k].clients.add(s.clientId);}); const categoryBreakdown=Object.values(categoryMap).map(x=>({...x,clients:x.clients.size,avgTicket:x.transactions?x.revenue/x.transactions:0,share:revenue?x.revenue/revenue*100:0})).sort((a,b)=>b.revenue-a.revenue);
//  const sellerBreakdown=group('seller'); const clientBreakdown=group('clientId',clientLookup).slice(0,10); const productBreakdown=group('productId',productLookup); const channelBreakdown=group('mediaOrigin').filter(x=>key(x.name)!=='desconocido');
//  const monthsWithSales=history.filter(x=>x.sales>0);const best=monthsWithSales.length?[...monthsWithSales].sort((a,b)=>b.sales-a.sales)[0].month:'Sin datos';const weakest=monthsWithSales.length?[...monthsWithSales].sort((a,b)=>a.sales-b.sales)[0].month:'Sin datos'; const newClients=newCustomerIdsForPeriod(sales,p).size; const recurrentClients=customerIdsByType(current,s=>s.isRepurchase).size;
//  const ytd=sales.filter(s=>s.date.getFullYear()===p.year&&s.date<=p.end); const lastYtd=sales.filter(s=>s.date.getFullYear()===p.year-1&&s.date.getMonth()<=p.endMonth-1);
//  return {company:'Casa Margot',meta:{selectedYear:p.year,selectedMonth:p.startMonth,selectedMonthLabel:p.label,previousMonthLabel:prev.label,years:yearsFrom(sales,clients),periodType:p.type,periodIndex:p.index},period:{...p,start:p.start.toISOString(),end:p.end.toISOString()},kpis:{monthlySales:revenue,salesGrowth:diffPct(revenue,previousRevenue),transactions:current.length,transactionGrowth:diffPct(current.length,prior.length),avgTicket,avgTicketGrowth:diffPct(avgTicket,previousTicket),uniqueClients:unique(current,'clientId'),newClients,recurrentClients,goalCompletion:null,monthlyGoal:null,gapVsGoal:null,ytdSales:salesSum(ytd),ytdGrowth:diffPct(salesSum(ytd),salesSum(lastYtd)),annualGoal:null},history,goalBars:history.map(x=>({...x,goal:null,completion:null,gap:null})),categoryBreakdown,productBreakdown,channelBreakdown,sellerBreakdown,clientBreakdown,seasonality:{bestMonth:best,weakestMonth:weakest,topCategory:categoryBreakdown[0]||null,topSeller:sellerBreakdown[0]||null},availability:{goals:false,channels:channelBreakdown.length>0,promotions:current.some(s=>s.promotionId),margin:false},insights:buildSalesInsightsCasa({revenueGrowth:diffPct(revenue,previousRevenue),avgTicketGrowth:diffPct(avgTicket,previousTicket),seller:sellerBreakdown[0],product:productBreakdown[0],channelAvailable:channelBreakdown.length>0})};
// }
// function buildSalesInsightsCasa({revenueGrowth,avgTicketGrowth,seller,product,channelAvailable}){const a=[];a.push({type:revenueGrowth>=0?'success':'danger',title:revenueGrowth>=0?'Ventas arriba del periodo anterior':'Ventas por debajo del periodo anterior',text:`La venta cambió ${Math.abs(revenueGrowth).toFixed(1)}% frente a la ventana comparable anterior.`});if(product)a.push({type:'success',title:'Producto líder',text:`${product.name} concentra ${product.share.toFixed(1)}% del ingreso del periodo.`});if(seller)a.push({type:seller.share>50?'warning':'success',title:'Concentración comercial',text:`${seller.name} representa ${seller.share.toFixed(1)}% de la venta del periodo.`});a.push({type:channelAvailable?'success':'warning',title:channelAvailable?'Origen de ventas disponible':'Canal de llegada por activar',text:channelAvailable?'Ya es posible comparar ventas por medio de origen.':'Casa Margot todavía no registra MEDIO_ORIGEN de forma confiable; al hacerlo se desbloqueará este cruce.'});return a.slice(0,4);}
...
// ----> LAST FUNCTIONAL SEP24/26 @ 10:50 <----
...
//       contactRows,
//       effortRows,
//       sales: normalizeSales(saleRows),
//       clients: normalizeClients(clientRows),
...
// async function getClientsAnalytics(query={}){
//  const p=periodFromQuery(query), prev=previousPeriod(p);
//  const {sales,clients}=await loadAll();
//  const current=sales.filter(s=>inPeriod(s.date,p)), prior=sales.filter(s=>inPeriod(s.date,prev));
//  const statusCounts={Activo:0,Dormido:0,Inactivo:0,'Sin compra':0};
//  const salesByClient = groupSalesByClient(sales);
//  const latestCurrent = buildLatestPurchaseMap(sales, p.end);
