...
const money=(n=0)=>new Intl.NumberFormat('es-MX',{style:'currency',currency:'MXN',minimumFractionDigits:2,maximumFractionDigits:2}).format(Number(n)||0);
...
export default function ProductosPromocionesScreen(){
 ...
 useEffect(()=>{const controller=new AbortController();setLoading(true);getCasaMargotProducts(period,{signal:controller.signal}).then(setData).catch(e=>{if(e.name!=='AbortError'){console.error(e);setData(fallback)}}).finally(()=>{if(!controller.signal.aborted)setLoading(false)});return()=>controller.abort()},[period.type,period.index,period.year]);
 ...
 return <main className="appShell"><AppSidebar/><div className="dashboard secondaryDashboard">
  ...
  <div className="secondaryGrid">
   ...
   <section className="panel"><div className="panelTitle"><div><h3>Mix por categoría</h3><p>Qué familias concentran la venta.</p></div></div><div className="chartFrame chartFrameTall"><ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}><BarChart data={data.categories||[]} layout="vertical" margin={{top:8,right:12,bottom:8,left:0}}><CartesianGrid horizontal={false} strokeDasharray="4 6"/><XAxis type="number" tickFormatter={v=>`$${Math.round(v/1000)}k`}/><YAxis type="category" dataKey="name" width={110}/><Tooltip formatter={v=>money(v)}/><Bar dataKey="revenue" fill="#4d9661" radius={[0,7,7,0]}/></BarChart></ResponsiveContainer></div></section>
...
// ----> LAST FUNCTIONAL SEP24/26 @ 13:50 <----
