...
async function getJson(url,msg,{signal}={}){const r=await fetch(url,{signal});if(!r.ok)throw new Error(msg);return r.json();}
...
export async function getIntelligenceOverview(q={}, options={}){return getJson(`${API_URL}/api/intelligence/overview?${paramsFor(q)}`,'No se pudo cargar la inteligencia esfuerzo-resultado',options);}
export async function getCasaMargotProducts(q={}, options={}){return getJson(`${API_URL}/api/intelligence/products?${paramsFor(q)}`,'No se pudo cargar Productos y Promociones',options);}
export async function getCasaMargotClients(q={}, options={}){return getJson(`${API_URL}/api/intelligence/clients?${paramsFor(q)}`,'No se pudo cargar Clientes',options);}
export async function getCasaMargotSales(q={}, options={}){return getJson(`${API_URL}/api/intelligence/sales?${paramsFor(q)}`,'No se pudo cargar Ventas',options);}




// ----> LAST FUNCTIONAL SEP 24/26 @ 16:25 <----
